...
const handleRegister = async () => {
    try {
        ...
        error.value = err.response?.data?.error || 'เกิดข้อผิดพลาดในการสมัครสมาชิก'
    } finally {
        loading.value = false
    }
}
    ...
    <div class="w-screen h-screen relative">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <div class="bg-blue-50 rounded-md p-10 shadow-lg shadow-blue-500/50">
                <h1 class="text-2xl font-bold text-center mb-5">สมัครสมาชิก</h1>
                ...
                <form @submit.prevent="handleRegister" class="mb-8 font-semibold space-y-4">

                    <!-- Email field -->
                    <div class="flex items-center gap-4">
                        <label for="email" class="w-32 shrink-0">Email:</label>
                        <input id="email" v-model="formData.email" type="email" :disabled="loading"
                    ...
                    <!-- Username field -->
                    <div class="flex items-center gap-4">
                        <label for="username" class="w-32 shrink-0">Username:</label>
                        <input id="username" v-model="formData.username" type="text" :disabled="loading"
                            class="flex-1 px-3 py-2 border border-blue-300 rounded-sm focus:border-blue-800 focus:outline-none focus:ring-1 focus:ring-blue-800 disabled:bg-gray-100"
                            required>
                    </div>
                    ...
                    <button type="submit" :disabled="loading"
                        class="block w-full p-2 bg-gradient-to-r from-cyan-500 to-blue-500 border rounded-md text-white font-semibold cursor-pointer hover:from-cyan-600 hover:to-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        {{ loading ? 'กำลังสมัครสมาชิก...' : 'สมัครสมาชิก' }}
                    </button>
                </form>

                <div class="text-end text-red-500 font-semibold hover:underline mt-4">
